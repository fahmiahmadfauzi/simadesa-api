...
{
		$stmt=$this->con->prepare("INSERT id FROM devices WHERE email=?");
...
class DbOperation
{
    ...
    public function registerDevice($email,$token){
        if(!$this->isEmailExist($email)){
            $stmt = $this->con->prepare("INSERT INTO device (nik, token) VALUES (?,?) ");
            $stmt->bind_param("ss",$email,$token);
            if($stmt->execute())
    ...
    private function isEmailexist($email){
        $stmt = $this->con->prepare("SELECT id FROM device WHERE nik = ?");
        $stmt->bind_param("s",$email);
        $stmt->execute();
        $stmt->store_result();
        $num_rows = $stmt->num_rows;
    ...
    public function getTokenByEmail($email){
        $stmt = $this->con->prepare("SELECT token FROM device WHERE nik = ?");
        $stmt->bind_param("s",$email);
        $stmt->execute(); 
        $result = $stmt->get_result()->fetch_assoc();
        return array($result['token']);        
    ...
    public function getAllDevices(){
        $stmt = $this->con->prepare("SELECT * FROM device");
        $stmt->execute();
        $result = $stmt->get_result();
        return $result; 
    }
